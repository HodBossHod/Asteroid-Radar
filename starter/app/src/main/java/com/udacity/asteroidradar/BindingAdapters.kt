...
fun bindImageOfTheDay(imgView:ImageView,pictureOfDay: PictureOfDay?){
    pictureOfDay?.let {
        Picasso.with(imgView.context).load(pictureOfDay.url).into(imgView)
...
fun bindContentDescription(imageView: ImageView, isSafe: Boolean?) {
    isSafe?.let {
        when(isSafe){
